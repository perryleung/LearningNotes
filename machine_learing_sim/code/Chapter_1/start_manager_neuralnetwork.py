...
def NN4py():
    ...
    training_data_file = open("../Datasets/Breast-Cancer/breast-cancer-train-new.csv","r")
    training_data_list = training_data_file.readlines()
    training_data_file.close()

    # load the mnist test data CSV file into a list
    testing_data_file = open("../Datasets/Breast-Cancer/breast-cancer-test-new.csv","r")
    ...
    for e in range(epochs):
	    ...
	    '''
            ...
            inputs = (np.asfarray(all_values[1:3]) / 10.0 * 0.99) + 0.01
	    # create the target output values (all 0.01, except the desired label which is 0.99)
	    targets = np.zeros(output_nodes) + 0.01
	    # all_values[0] is the target label for this record
	    # 下标是目标数字，也是输出层10个节点对应10个数字中想要激活的那个节点
	    targets[int(all_values[3])] = 0.99
	...
	correct_label = int(all_values[3])
